//
//  BaiduPan SVIP Direct Link Helper - TrollStore Edition v10.1
...
// ========== v10.1 CORE: Auto-click renamed file ==========
static void autoClickRenamedFile(NSString *ipaName) {
    if (!ipaName) return;
    DLog(@"Auto-clicking renamed file: %@", ipaName);
    ...
    // Method 1: Find UITableView and match by data source
    UITableView *tableView = (UITableView *)findViewRecursively(vc.view, [UITableView class]);
    if (tableView) {
        DLog(@"Found tableView, searching for file");
        ...
        if (dataSource) {
            ...
            if (fileList && [fileList isKindOfClass:[NSArray class]]) {
                ...
                for (NSUInteger i = 0; i < [(NSArray *)fileList count]; i++) {
                    if (matchFileByName([(NSArray *)fileList objectAtIndex:i], ipaName)) {
                        targetIndex = i;
                        break;
                    }
                }
                ...
                if (targetIndex != NSNotFound) {
                    ...
                    if (cell) {
                        triggerCellActionByClassList(cell, ipaName);
                        showToast(@"已自动点击文件");
                        return;
                    }

                    // Fallback: call delegate didSelect
    ...
    // Method 2: Find UICollectionView
    UICollectionView *collectionView = (UICollectionView *)findViewRecursively(vc.view, [UICollectionView class]);
    if (collectionView) {
        DLog(@"Found collectionView, searching for file");
            ...
            for (NSInteger i = 0; i < count; i++) {
                ...
                @try {
                    ...
                    if (cell) {
                        ...
                        @try { cellName = [item valueForKey:@"server_filename"]; } @catch (NSException *e) {}
                        if (!cellName) @try { cellName = [item valueForKey:@"name"]; } @catch (NSException *e) {}
                        if ([cellName isEqualToString:ipaName]) {
    ...
    // Method 3: Find label with file name and simulate touch on its parent cell
    DLog(@"Falling back to label search");
    UIWindow *window = nil;
    if (@available(iOS 13.0, *)) {
        for (UIWindowScene *scene in [[UIApplication sharedApplication] connectedScenes]) {
            if (scene.activationState == UISceneActivationStateForegroundActive) {
    ...
    if (!window) window = [[UIApplication sharedApplication] keyWindow];
    if (window) {
        UIView *label = findLabelWithText(window, ipaName, 0);
        if (label) {
            DLog(@"Found label with file name, traversing up to find cell");
            ...
            while (parent && depth < 15) {
                if ([parent isKindOfClass:[UITableViewCell class]] || [parent isKindOfClass:[UICollectionViewCell class]]) {
                    triggerCellActionByClassList(parent, ipaName);
                    showToast(@"已自动点击文件");
                    return;
                }
                parent = parent.superview;
            ...
            // If no cell found, just touch the label's immediate parent
            if (label.superview) {
                simulateTouchOnView(label.superview);
                showToast(@"已自动点击文件");
                return;
            }
        }
    ...
    DLog(@"Auto-click failed, waiting for manual tap");
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    ...
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        // Try auto-click again after scroll
                        autoClickRenamedFile(ipaName);
...
static void onFloatButtonTap(void) {
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"BaiduPan Troll v10.1"
...
static void baiduPanTrollInit(void) {
    DLog(@"BaiduPan Troll v10.1 loaded - Auto-Click Edition");
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        showFloatButton();
        autoDetectPathAndToken();
    });
