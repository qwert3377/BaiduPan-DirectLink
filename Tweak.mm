//
// BaiduPan SVIP Direct Link Helper - v13.0
// Fix: Use UIApplication as button target, NSNotificationCenter for events
//
...
static void onFloatButtonAction(void);
...
static void showFloatButton(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        // 使用 UIApplication.sharedApplication 作为 target，发送通知
        [gFloatButton addTarget:[UIApplication sharedApplication] action:@selector(description) forControlEvents:UIControlEventTouchUpInside];

        UIPanGestureRecognizer *pan = [[UIPanGestureRecognizer alloc] initWithTarget:[UIApplication sharedApplication] action:@selector(description)];
...
%ctor {
    @autoreleasepool {
        // 监听按钮点击通知（通过 UIApplication 发送）
        [[NSNotificationCenter defaultCenter] addObserverForName:@"BaiduPanButtonTapped" object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *note) {
            onFloatButtonAction();
        }];
