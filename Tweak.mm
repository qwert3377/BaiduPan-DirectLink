//
//  BaiduPan SVIP Direct Link Helper - TrollStore Edition v8.5.5
//  Fix: forceRefreshFileList now fully simulates user pull-to-refresh gesture sequence
//       (beginDragging -> scroll -> endDragging -> willDecelerate:NO)
//       Previous versions only set contentOffset, missing the full gesture lifecycle.
//  Fix: Added UITableView/UICollectionView reloadData as fallback
//  Fix: Path detection via title concatenation (v8.5.4)
//  Fix: pollForFileExistence for reliable post-rename link fetching
//
...
// v8.5.5: Full gesture simulation for pull-to-refresh
static void simulatePullToRefreshOnScrollView(UIScrollView *scrollView) {
    ...
    // Step 1: Begin dragging (touch down + drag)
    ...
    // Step 2: Drag to pull position (no animation, instant)
    [scrollView setContentOffset:CGPointMake(originalOffset.x, -150) animated:NO];

    // Step 3: Scroll did scroll
    ...
    // Step 4: End dragging (finger lift)
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        // Step 5: If decelerating, trigger didEndDecelerating
        ...
        // Step 6: Restore contentOffset with animation after refresh completes
...
static void tryRefreshOnScrollView(UIScrollView *scrollView) {
    ...
    // 3. Full gesture simulation (v8.5.5)
    simulatePullToRefreshOnScrollView(scrollView);

    // 4. EGO / BDWallet / BDPan refresh subviews
...
static void refreshVC(UIViewController *vc) {
    ...
    // Try UITableView/UICollectionView reloadData
...
static void onFloatButtonTap(void) {
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"BaiduPan Troll v8.5.5"
...
static void baiduPanTrollInit(void) {
    DLog(@"BaiduPan Troll v8.5.5 loaded");
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        showFloatButton();
        autoDetectPathAndToken();
    });
