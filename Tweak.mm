//
//  BaiduPan SVIP Direct Link Helper - TrollStore Edition v9.7
//  Fix: Hook 文件列表 Cell 的点击事件，直接触发 App 原生下载
//       不再依赖寻找下载管理器，而是模拟用户点击"下载"按钮的行为
//
...
static void triggerDownloadBySimulatingUserAction(NSDictionary *fileMeta) {
    ...
    // 方法1: 尝试调用 VC 的 tableView:didSelectRowAtIndexPath: 并构造对应 indexPath
    // 先找到 tableView
    ...
    // 方法2: 尝试找到文件列表的数据源，直接操作
    if (!tableView) {
        // 遍历所有子视图找 tableView
        NSArray *allSubviews = nil;
        @try {
            allSubviews = [vc.view subviews];
        } @catch (NSException *e) {}
    ...
    // 方法3: 尝试通过 dataSource 获取文件列表并找到对应 cell
    if (tableView) {
        DLog(@"Found tableView");
        id dataSource = tableView.dataSource;
        if (dataSource) {
            DLog(@"DataSource class: %@", NSStringFromClass([dataSource class]));
            ...
            if (fileList && [fileList isKindOfClass:[NSArray class]]) {
                ...
                if (targetIndex != NSNotFound) {
                    ...
                    // 尝试调用 delegate 的 didSelectRowAtIndexPath
    ...
    // 方法4: 尝试直接调用 VC 的下载相关方法（带 fileMeta）
    NSArray *vcSelectors = @[
        @"downloadFile:",
    ...
    // 方法5: 尝试 navigationController 的 viewControllers
...
static void onFloatButtonTap(void) {
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"BaiduPan Troll v9.7"
...
static void baiduPanTrollInit(void) {
    DLog(@"BaiduPan Troll v9.7 loaded");
